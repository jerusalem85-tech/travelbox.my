...
<head>
<meta charset="utf-8">
<title>Voucher – {{ $trip->trip_number }}</title>
<style>
...
.service-ref{font-size:7pt;color:#9ca3af}
</style>
</head>
<body>
...
<div class="header">
    <h1>TravelBox</h1>
    <p style="font-size:11pt;font-weight:600">Trip Voucher</p>
    <p>{{ $trip->trip_number }} &bull; <span class="badge badge-{{ $trip->status }}">{{ str_replace('_', ' ', ucfirst($trip->status)) }}</span></p>
</div>
...
<div class="section">
    <h3>Flights</h3>
    <table>
        <thead><tr><th>Airline</th><th>Flight</th><th>From</th><th>To</th><th>Departure</th><th>Arrival</th><th>Ref</th></tr></thead>
        <tbody>
        ...
        <tr>
            ...
            <td><span class="service-ref">{{ $fs->booking_reference ?: '—' }}</span></td>
        </tr>
        @endforeach
        </tbody>
    </table>
...
<div class="section">
    <h3>Hotels</h3>
    <table>
        <thead><tr><th>Hotel</th><th>City</th><th>Check-in</th><th>Check-out</th><th>Room</th><th>Rooms</th><th>Ref</th></tr></thead>
        <tbody>
        ...
        <tr>
            ...
            <td><span class="service-ref">{{ $hb->booking_reference ?: '—' }}</span></td>
        </tr>
        @endforeach
        </tbody>
    </table>
...
<div class="section">
    <h3>Transfers</h3>
    <table>
        <thead><tr><th>Type</th><th>Pickup</th><th>Dropoff</th><th>Date/Time</th><th>Vehicle</th><th>Ref</th></tr></thead>
        <tbody>
        ...
        <tr>
            ...
            <td><span class="service-ref">{{ $tb->booking_reference ?: '—' }}</span></td>
        </tr>
        @endforeach
        </tbody>
    </table>
...
<div class="section">
    <h3>Visas</h3>
    <table>
        <thead><tr><th>Country</th><th>Type</th><th>Status</th><th>Expected</th></tr></thead>
        <tbody>
        @foreach ($trip->visaApplications as $v)
        <tr><td>{{ $v->country }}</td><td>{{ str_replace('_', ' ', ucfirst($v->visa_type ?? 'tourist')) }}</td><td>{{ $v->status }}</td><td>{{ $v->expected_delivery?->format('M d, Y') ?: '—' }}</td></tr>
        @endforeach
        </tbody>
    </table>
</div>
...
<div class="section">
    <h3>Insurance</h3>
    <table>
        <thead><tr><th>Policy</th><th>Type</th><th>Coverage</th><th>Valid From</th><th>Valid To</th></tr></thead>
        <tbody>
        @foreach ($trip->insurancePolicies as $ins)
        <tr><td>{{ $ins->policy_number ?: '—' }}</td><td>{{ str_replace('_', ' ', ucfirst($ins->type ?? 'travel')) }}</td><td>{{ $ins->coverage_details ?: '—' }}</td><td>{{ $ins->start_date?->format('M d, Y') }}</td><td>{{ $ins->end_date?->format('M d, Y') }}</td></tr>
        @endforeach
        </tbody>
    </table>
</div>
...
<div class="section">
    ...
    <table>
        <thead><tr><th>Service</th><th>Type</th><th>Date</th><th>Cost</th></tr></thead>
        <tbody>
        ...
        <tr>
            ...
            <td>{{ $o->selling_price ? number_format($o->selling_price, 2) : '—' }}</td>
        </tr>
        @endforeach
        </tbody>
    </table>
...
<div class="footer">
    ...
    <p style="font-size:7pt">This is a computer-generated voucher. Please present all service vouchers to the respective providers.</p>
</div>
