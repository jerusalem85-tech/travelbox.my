    ...
    <div className="space-y-6">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-auto">
            <div className="flex items-center justify-between p-6 border-b">
              <h2 className="text-lg font-semibold">{editingId ? "تعديل عميل" : "إضافة عميل جديد"}</h2>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-gray-100">
          ...
          <p className="text-gray-500">{search ? "لا توجد نتائج للبحث" : "لا يوجد عملاء بعد"}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((client) => (
            <div key={client.id} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              ...
              <p className="text-xs text-gray-400 mt-3">{new Date(client.createdAt).toLocaleDateString("ar-EG")}</p>
            </div>
          ))}
        </div>
      )}
