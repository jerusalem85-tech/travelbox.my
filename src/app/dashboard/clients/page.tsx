...
export default function ClientsPage() {
  ...
  const filtered = clients.filter((c) => c.name.includes(search) || c.phone?.includes(search) || c.email?.includes(search));
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage client database</p>
        </div>
        <button onClick={() => { setForm(emptyClient); setEditingId(null); setShowForm(true); }}
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Search for a client..." value={search} onChange={(e) => setSearch(e.target.value)}
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-gray-100">
          ...
          <p className="text-gray-500">{search ? "No search results" : "No clients yet"}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((client) => (
            <div key={client.id} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-3">
                <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center">
                  <span className="text-blue-600 font-bold text-sm">{client.name.charAt(0)}</span>
                </div>
              ...
              <div className="mt-2 space-y-1">
                {client.phone && <p className="flex items-center gap-2 text-sm text-gray-500"><Phone className="w-3.5 h-3.5" />{client.phone}</p>}
                {client.email && <p className="flex items-center gap-2 text-sm text-gray-500"><Mail className="w-3.5 h-3.5" />{client.email}</p>}
              </div>
              <p className="text-xs text-gray-400 mt-3">{new Date(client.createdAt).toLocaleDateString("en-US")}</p>
            </div>
          ))}
        </div>
      )}
