...
const categories = ["Rent", "Salaries", "Electricity & Water", "Internet", "Transport", "Marketing", "Office", "Travel", "Other"];

const emptyExpense = { description: "", category: "Other", amount: 0, date: new Date().toISOString().split("T")[0] };
    ...
    <div className="space-y-6">
          ...
          <div className="space-y-3">
            {categoryTotals.map((cat) => (
              <div key={cat.category} className="flex items-center gap-4">
                ...
                <span className="text-sm font-semibold text-gray-900 w-28 text-left">{cat.total.toLocaleString()} $</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit" className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl transition-colors">Add</button>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-gray-100">
          ...
          <p className="text-gray-500">{search ? "No search results" : "No expenses yet"}</p>
        </div>
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="text-right px-6 py-4 text-xs font-medium text-gray-500 uppercase">Category</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map((exp) => (
                  <tr key={exp.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-6 py-4 text-sm font-semibold text-red-600">{Number(exp.amount).toLocaleString()} $</td>
                    <td className="px-6 py-4 text-sm text-gray-500">{new Date(exp.date).toLocaleDateString("en-US")}</td>
