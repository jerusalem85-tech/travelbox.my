    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          ...
          <p className="text-2xl font-bold text-green-600">{totalRevenue.toLocaleString()} ر.م</p>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <div className="flex items-center gap-3 mb-3">
          ...
          <p className="text-2xl font-bold text-yellow-600">{(totalPending + totalOverdue).toLocaleString()} ر.م</p>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <div className="flex items-center gap-3 mb-3">
          ...
          <p className="text-2xl font-bold text-red-600">{totalExpenses.toLocaleString()} ر.م</p>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <div className="flex items-center gap-3 mb-3">
          ...
          <p className={`text-2xl font-bold ${netProfit >= 0 ? "text-blue-600" : "text-red-600"}`}>{netProfit.toLocaleString()} ر.م</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">المصروفات حسب الفئة</h2>
          {Object.keys(categoryBreakdown).length === 0 ? (
            <div className="text-center py-8 text-gray-400"><BarChart3 className="w-10 h-10 mx-auto mb-2 opacity-50" /><p className="text-sm">لا توجد بيانات</p></div>
          ) : (
              ...
              {Object.entries(categoryBreakdown).sort(([, a], [, b]) => b - a).map(([category, amount]) => (
                <div key={category} className="space-y-1">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium text-gray-900">{amount.toLocaleString()} ر.م</span>
                  </div>
      ...
      <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">ملخص الفواتير</h2>
